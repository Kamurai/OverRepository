...
BEGIN
	...
	delete from PlayOverLists where MasterUserIndex = @intUserIndex and VideoGameIndex IN (
	...
	--select * from VideoGames, PlayOverLists where VideoGames.TargetIndex = PlayOverLists.VideoGameIndex and PlayOverLists.MasterUserIndex = 0 order by OrderRank;


	select *, ROW_NUMBER() Over(order by OrderRank) as RowNum INTO #NumOne from PlayOverLists where MasterUserIndex = 0; 
	--select * from #NumOne;

	select *, ROW_NUMBER() Over(order by OrderRank) as RowNum INTO #NumTwo from PlayOverLists where MasterUserIndex = 0; 
	--select * from #NumTwo;
	...
	(
		...
		FROM PlayOverLists where MasterUserIndex = @intUserIndex
	)
