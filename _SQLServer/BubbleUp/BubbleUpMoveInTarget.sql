...
AS
BEGIN
	DECLARE @intParentBoxIndex	int = (SELECT TOP 1 ParentBoxIndex	FROM BOXES WHERE UserIndex = @intUserIndex AND BoxIndex = @intMovingTargetIndex);
	DECLARE @intChildCount		int = (SELECT COUNT(BoxIndex)		FROM BOXES WHERE UserIndex = @intUserIndex AND ParentBoxIndex = @intParentBoxIndex);
