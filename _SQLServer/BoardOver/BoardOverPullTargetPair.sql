...
BEGIN
	...
	BEGIN
		...
		SET @OrderCount = (
		select count(ListIndex) from BoardOverLists where MasterUserIndex = @intUserIndex and (UpLock = 0 or DownLock = 0)
		...
		BEGIN
			...
			BEGIN
				...
				SET @TargetIndex = (select top 1 ListIndex from BoardOverLists where MasterUserIndex = @intUserIndex and (UpLock = 0 or DownLock = 0) order by newid());
			END
			ELSE
			BEGIN
				--//request random non-locked Target from personal list
			...
			--//find a record to compare to the one we have
				--//if order is 0 or equal to count
					--//there are BoardGames left in the global list
			if ( (select count(MasterUserIndex) from BoardOverLists 
	...
	BEGIN
		...
		where BoardOverUsers.MasterUserIndex = @intUserIndex order by newid();
	END
